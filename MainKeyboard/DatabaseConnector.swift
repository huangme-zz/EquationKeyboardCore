...
class DatabaseConnector {
  ...
  func getLatexRenderedURL(latexExp : String) -> String?{
    ...
    let url_to_request = "http://ec2-54-146-55-27.compute-1.amazonaws.com:3001/secret/equation_keyboard/api/v1/render"
    let url:NSURL = NSURL(string: url_to_request)!
    let session = URLSession.shared
    let request = NSMutableURLRequest(url: url as URL)
    ...
    if (post_success) {
      return "http://ec2-54-146-55-27.compute-1.amazonaws.com:3001/secret/equation_keyboard/static/" + file_name
    } else {
      return nil
    }
  }
  ...
  func getLatexRenderedURL_large(latexExp : String) -> String?{
    ...
    let semaphore = DispatchSemaphore(value: 0)
    var post_success = false
    let url_to_request = "http://ec2-54-146-55-27.compute-1.amazonaws.com:3001/secret/equation_keyboard/api/v1/render_big"
    let url:NSURL = NSURL(string: url_to_request)!
    let session = URLSession.shared
    let request = NSMutableURLRequest(url: url as URL)
    request.httpMethod = "POST"
